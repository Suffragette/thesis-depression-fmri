%% tost_peredge_v25.m - PER-EDGE TOST on the v25 NeuroMark pipeline (sensitivity)
%% IDENTICAL statistics to tost_peredge.m; ONLY the input data differ (v25).
...
for s=1:72
    m=load(fullfile(ROOT,'output72_v25','nmark72v25_postprocess_results',sprintf('nmark72v25_post_process_sub_%03d.mat',s)));
...
% ===== SIGN CHECK: within-DMN (1-16 analogue) and DMN-ECN, depr vs HC =====
% Reports the DIRECTION explicitly (per-edge TOST above uses abs(d) and hides it).
% The paper's 1-16 within-DMN and 11-13 DMN-ECN are the mappable triple-network claims.
gS=[true(51,1);false(21,1)];
mWk=triu(true(numel(DMN)),1);
wdmn=nan(72,1); dmnecn=nan(72,1);
for s=1:72
    m=load(fullfile(ROOT,'output72_v25','nmark72v25_postprocess_results',sprintf('nmark72v25_post_process_sub_%03d.mat',s)));
    M=squeeze(m.fnc_corrs); M(isnan(M))=0;
    b1=M(DMN,DMN); wdmn(s)=mean(b1(mWk));
    b2=M(DMN,ECN); dmnecn(s)=mean(b2(:));
end
fprintf('\n===== v25 DIRECTION CHECK (depr vs HC) =====\n');
for pair={{'within-DMN',wdmn},{'DMN-ECN',dmnecn}}
    nm=pair{1}{1}; v=pair{1}{2};
    md=mean(v(gS)); mh=mean(v(~gS));
    [~,pp]=ttest2(v(gS),v(~gS));
    dir=''; if md>mh, dir='depr>HC'; else, dir='HC>depr'; end
    fprintf('%-10s: depr=%+.4f  HC=%+.4f  -> %s  p=%.3f\n', nm, md, mh, dir, pp);
end
fprintf('Compare to 2.2 (run tost_peredge.m): within-DMN was depr>HC there.\n');
fprintf('If direction differs across pipelines -> SIGN INSTABILITY (both n.s.).\n');
