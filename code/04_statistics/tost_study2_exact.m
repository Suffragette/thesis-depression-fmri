...
for k=1:size(C,1)
    ...
    % ---- EXACT TOST at the paper-reported dz (no grid approximation) ----
    raw_exact = pdz * s;

    pL_exact = 1 - tcdf((m + raw_exact)/se, nu);  % H0: mean <= -bound
    pU_exact =     tcdf((m - raw_exact)/se, nu);  % H0: mean >= +bound

    pT_exact = max(pL_exact, pU_exact);

    fprintf([' >> EXACT paper dz=%.3f: raw bound=%+.5f  ' ...
             'pL=%.4f  pU=%.4f  p_TOST=%.4f -> %s\n'], ...
             pdz, raw_exact, pL_exact, pU_exact, pT_exact, ...
             tern(pT_exact<ALPHA,'PAPER EFFECT EXCLUDED','not excluded'));
