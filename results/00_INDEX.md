...
| File | Theme | Type of evidence | What it shows |
|---|---|---|---|
| `nbs_primary.txt` | primary group comparison | Primary null test (FWER-controlled NBS) | Whether any connected subnetwork differs between depressed and controls over the triple-network. |
| `domain_level.txt` | primary group comparison | Coarse-network null test (t-tests + Bonferroni/FDR) | Whether the coarse DMN/ECN/salience domain-pairs differ between groups after correction. |
| `chance_audit.txt` | primary group comparison | Chance baseline (binomial; needs no data) | Whether the original's reported "significant" counts exceed what is expected by chance under no correction. |
| `perm_null.txt` | primary group comparison | Empirical permutation null (10,000 perms) | How many "significant" connections appear vs the chance expectation, accounting for inter-edge dependence. |
...
| `repro_1012.txt` | Study 2 dynamics | Targeted reproduction (paired tests + correlation) | Whether the central Study-2 finding (the "10-12" connection) and its five sub-claims reproduce. |
| `study2_longitudinal.txt` | Study 2 dynamics | Longitudinal pre–post (paired t, with CIs) | Pre-post connectivity changes for all four Study-2 groups, with confidence intervals (small n). |
| `robustness_loo.txt` | robustness | Robustness (leave-one-out + data QC) | Whether the null result is driven by any single participant or by data-quality issues. |
| `tost_equivalence.txt` | equivalence testing | Equivalence test (TOST, per connection) | For each connection, whether a difference as large as the one reported can be excluded (how much of the connectome the data can bound). |
